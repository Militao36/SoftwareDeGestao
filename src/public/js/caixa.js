      ...
      for (const item of data) {
        $("#tableCaixa tbody").append(`
              <tr onclick="selectGridCaixa('${item.uuid}')"> 
                  ...
                  <td>${item.valor.toLocaleString("pt-br", { style: "currency", currency: "BRL", }) || 0}</td>
              </tr>
        `);
      }
    });
...
// Começo geração relatório

//.toSQLDate()
let date = luxon.DateTime.local().setZone("America/Sao_Paulo");

getMov(date);

function getMov(date) {
  $("#tableMovDia tbody").empty()
  axios.post('/Caixa/movimentacao', { date: date.toSQLDate() })
    .then((response) => {
      const { caixa } = response.data
      console.log(caixa)
      for (const item of caixa) {
        $("#tableMovDia tbody").append(`
            <tr> 
                <td>${item.tipo}</td> 
                <td>${String(item.tipoMov).toUpperCase()}</td>
                <td>${item.valor.toLocaleString("pt-br", { style: "currency", currency: "BRL", }) || 0}</td>
            </tr>
        `);
      }
...
document.getElementById("atual").addEventListener("click", (e) => {
  e.preventDefault();
  date = luxon.DateTime.local().setZone("America/Sao_Paulo");
  getMov(date);
});
